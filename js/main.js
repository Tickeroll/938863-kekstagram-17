...
var percent=parseInt(scaleControlValue.value);
  percent-=25;
  if (percent > 0) {
...
scaleControlValue.onchange = function () {
  document.querySelector('.img-upload__preview').style.transform='scale(0.'+ parseInt(scaleControlValue.value)+")";
}
...
document.querySelector('.effect-level__pin').onmouseup = function (event) {
  effectLevel.value = parseInt(this.style.left);
...
console.log(document.querySelector('.img-upload__preview').style.filter);
}
document.querySelector('.img-upload__preview')
document.querySelector('.effect-level__pin').offsetWidth;
